...
export class AuthenticationApi {
  ...
  static registerVisitor(): Promise<{ token?: string }> {
    return HttpService.api.post(`/v1/authentication/register-visitor`)
  }
  ...
  static verifyEmail(token: string): Promise<void> {
    return HttpService.api.post(
      `/v1/authentication/verify-email?data=${encodeURIComponent(token)}`,
      {
